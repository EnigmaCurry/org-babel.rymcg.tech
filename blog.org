    ...
    (let ((default-directory my/blog-root))    
      ...
      (if (not (= 0 (call-process-shell-command 
        ...
        (if (not (= 0 (call-process-shell-command "git diff-index --quiet HEAD --")))
          ...
          ; Generate hugo static site in a temporary directory:
          (let ((temp-hugo-dir (make-temp-file "hugo-tmp"))
                (default-directory my/hugo-root))
            (if (not (= 0 (call-process-shell-command (concat "./go/bin/hugo -d " temp-hugo-dir))))
                (message (concat "Failed to create hugo site in " temp-hugo-dir))
