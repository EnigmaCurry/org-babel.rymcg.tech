...
** Install
 - Download [[https://raw.githubusercontent.com/EnigmaCurry/rymcg.tech/master/blog.org][blog.org]] to your =~/.emacs.d=. 
 - Put this in your own =~/.emacs.d/{init.el,config.org}=: 
...
These are the manual steps for building the website. Later on we'll
build up a script to automate all this, so all though the next bit is
a bit tedious, this should be much clearer conceptually how this thing
works:

 - Open up your new =~/.emacs.d/blog.org=, in Emacs.
 ...
 - Search and replace all the =~/git/rymcg.tech= directory references
   to make them your own local website git clone. (See caveat in
   [[Deficiencies]].)
 - Save the file. It's yours, remeber!
 - Now tangle all the org-babel code blocks: (aka. 'render all code blocks
   marked with =:tangle= to the specified external file.')
  - Type ==C-c C-v t== (or =M-x org-babel-tangle=)
 - Now run ox-hugo to export to markdown:
  - Type ==C-c C-e H A== (or =C-1 M-x org-hugo-export-subtree-to-md=)
 - Now run hugo to translate the markdown to the public html:
