...
graduated to using the automated script. Read the next section, and
then just run =M-x blog-publish= from now on. It will create a new
=gh-pages= branch and build and publish HTML to =blog/public=.
...
(use-package ox-hugo
  ...
  (defun blog-publish ()
    "Build and publish the blog to the gh-pages branch"
    (interactive)
    (let ((default-directory my/blog-root))    
      ...
      (if (not (= 0 (call-process-shell-command 
        ...
        (if (not (= 0 (call-process-shell-command "git diff-index --quiet HEAD --")))
          ...
          (call-process-shell-command "git branch -D gh-pages")
          ; Recreate gh-pages branch from scratch:
          (if (not (= 0 (call-process-shell-command "git rev-parse --verify gh-pages")))
              (progn
                (call-process-shell-command "git checkout --orphan gh-pages")
          ...
          (call-process-shell-command "git add blog/public/* && git commit -m gh-pages")
          ; Go back to master:
          (call-process-shell-command "git checkout master")
          ; Push
          (call-process-shell-command "git push -f origin gh-pages")))))
...
#+RESULTS: testguy
#+BEGIN_EXPORT html
<div class="block-header results">testguy - Last run: 2017-10-03 03:15:59 PM</div>
